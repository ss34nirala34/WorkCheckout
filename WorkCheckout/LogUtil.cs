...
{   // 添加System.Configuration.dll引用
    ...
    public static class LogUtil
    {
        ...
        private static string defaultPath = AppDomain.CurrentDomain.BaseDirectory + "Log";
        ...
        private static string FormatMsg(string msg, LogType logType)
        {
            ...
            string result;
            string header = string.Format("[{0}][{1} {2}] ", logType.ToString(), string.Format("{0:yyyy-MM-dd}", dt), string.Format("{0:HH:mm:ss}", dt));
            result = header + msg;
            return result;
        }
        ...
        {
            string result = string.Format("{0:yyyy-MM-dd}",DateTime.Now);
            if (logType != LogType.Option)
